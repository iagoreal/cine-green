    ...
    <div>
      <div class="w-12/12 p-5 md:w-4/12">
        ...
        <div class="my-2 flex items-center gap-4 text-sm text-white">
          ...
          <p class="font-base font-medium" v-else>
            {{ runningTime  }} min
          </p>
        </div>
        ...
        <div class="w-fit" v-if="!isAlreadyFavorite">
          <prime-button @click="storeFavorite()" severity="contrast"
            ...
            label="Favoritar" icon="pi pi-heart-fill " />
        </div>
        <div class="w-fit text-white" v-else>
          <prime-button @click="storeFavorite()" severity="danger"
            ...
            label="Favorito" icon="pi pi-heart-fill " />
        </div>
      </div>
    </div>
  </div>
...
function removeFavorite(): void{
  favorites.value = favorites.value.filter(favorite => favorite.id !== movieOrTvShow.value.id)
}
